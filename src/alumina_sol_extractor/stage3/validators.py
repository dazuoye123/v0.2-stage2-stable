...
def validate_duplicate_evidence_ids(record: PaperExtractionRecord) -> list[dict[str, Any]]:
    """Warn when evidence_ids still collide after post-processing."""
    ...
    for evidence_id, count in Counter(evidence_ids).items():
        if count > 1:
            issues.append(
                {
                    "type": "duplicate_evidence_id_warning",
...
def validate_evidence_figure_id_alignment(record: PaperExtractionRecord) -> list[dict[str, Any]]:
    """Warn when a figure evidence_id does not match its figure_id base token."""
    ...
    for obj in record.evidence_objects:
        ...
        if not figure_id:
            continue
        expected_prefix = f"{figure_id}__ev"
        if evidence_id != figure_id and not evidence_id.startswith(expected_prefix):
            issues.append(
                {
                    "type": "evidence_figure_id_mismatch_warning",
                    "evidence_id": evidence_id,
                    "figure_id": figure_id,
                }
            )
...
def validate_core_parameter_without_evidence(
    record: PaperExtractionRecord,
    ontology: OntologyMap,
) -> list[dict[str, Any]]:
    """Warn when core additional ParameterRecords do not carry evidence refs."""

    issues: list[dict[str, Any]] = []
    for scope, parameter_record in _iter_additional_parameter_records(record):
        canonical_key = parameter_record.canonical_key
...
def build_quality_flags(record: PaperExtractionRecord, validation_errors: Iterable[dict[str, Any]]) -> list[str]:
    ...
    flag_map = {
        ...
        "duplicate_evidence_id_warning": "duplicate_evidence_ids_detected",
        "evidence_ref_warning": "evidence_reference_warnings",
        "evidence_figure_id_mismatch_warning": "evidence_figure_id_mismatch_detected",
        "core_parameter_without_evidence_warning": "core_parameters_without_evidence",
...
def _iter_additional_parameter_records(record: PaperExtractionRecord):
    if record.global_constants:
        for parameter_record in record.global_constants.additional_parameter_records:
            yield "global_constants.additional_parameter_records", parameter_record
    for series in record.experiment_series:
        if series.series_constants:
            scope = f"series_constants:{series.series_id}"
            for parameter_record in series.series_constants.parameter_records:
