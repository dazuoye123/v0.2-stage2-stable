...
def test_stage3_batch_custom_subdir_writes_to_stage3_full(monkeypatch, tmp_path: Path) -> None:
    module = _load_script_module()
    manifest = tmp_path / "source_manifest.csv"
    markdown_dir = tmp_path / "markdown"
    outputs_dir = tmp_path / "outputs"
    ...
    markdown_path = markdown_dir / "fiber_process" / "paper1.md"
    paper_output_dir = outputs_dir / "fiber_process" / "paper1"
    stage3_dir = paper_output_dir / "stage3_full"
    stage3_dir.mkdir(parents=True, exist_ok=True)
    markdown_path.parent.mkdir(parents=True, exist_ok=True)
    markdown_path.write_text("# 2.2 实验部分\n正文\n", encoding="utf-8")
    manifest.write_text(
    ...
    def fake_run_stage3(**kwargs):
        assert kwargs["stage3_subdir"] == "stage3_full"
        summary = {
            "stage3_mode": "full",
            "stage3_dspy": "enabled",
            "cleaned_body_char_count": 1200,
    ...
    row = result["rows"][0]
    assert row["status"] == "success"
    assert row["stage3_dir"].endswith("stage3_full")


def test_stage3_batch_custom_subdir_does_not_skip_based_on_other_subdir(monkeypatch, tmp_path: Path) -> None:
    module = _load_script_module()
    manifest = tmp_path / "source_manifest.csv"
    markdown_dir = tmp_path / "markdown"
    outputs_dir = tmp_path / "outputs"
    ...
    markdown_path = markdown_dir / "fiber_process" / "paper1.md"
    paper_output_dir = outputs_dir / "fiber_process" / "paper1"
    stage3_full_dir = paper_output_dir / "stage3_full"
    stage3_twopass_dir = paper_output_dir / "stage3_twopass"
    stage3_full_dir.mkdir(parents=True, exist_ok=True)
    stage3_twopass_dir.mkdir(parents=True, exist_ok=True)
    markdown_path.parent.mkdir(parents=True, exist_ok=True)
    markdown_path.write_text("# 2.2 实验部分\n正文\n", encoding="utf-8")
    manifest.write_text(
        "source_id,category,paper_id_guess,markdown_expected_path\n"
    ...
    (stage3_full_dir / "stage3_summary.json").write_text(
        json.dumps({"stage3_mode": "full", "schema_valid": True}, ensure_ascii=False),
        encoding="utf-8",
    )

    def fake_run_stage3(**kwargs):
        assert kwargs["stage3_subdir"] == "stage3_twopass"
        summary = {
            "stage3_mode": "two-pass",
            "stage3_dspy": "enabled",
        ...
        (stage3_twopass_dir / "stage3_summary.json").write_text(json.dumps(summary, ensure_ascii=False), encoding="utf-8")
        (stage3_twopass_dir / "stage3_procedure_sections.json").write_text("[]", encoding="utf-8")
        return _FakeStage3Result(stage3_twopass_dir, summary)
...
def test_stage3_batch_skip_is_scoped_to_selected_subdir(tmp_path: Path) -> None:
    module = _load_script_module()
    manifest = tmp_path / "source_manifest.csv"
    markdown_dir = tmp_path / "markdown"
    outputs_dir = tmp_path / "outputs"
    report_dir = tmp_path / "reports"
    markdown_path = markdown_dir / "fiber_process" / "paper1.md"
    stage3_twopass_dir = outputs_dir / "fiber_process" / "paper1" / "stage3_twopass"
    stage3_twopass_dir.mkdir(parents=True, exist_ok=True)
    markdown_path.parent.mkdir(parents=True, exist_ok=True)
    markdown_path.write_text("# 2.2 实验部分\n正文\n", encoding="utf-8")
    manifest.write_text(
    ...
    (stage3_twopass_dir / "stage3_summary.json").write_text(
        json.dumps(
            {
                "stage3_mode": "two-pass",
                "schema_valid": True,
