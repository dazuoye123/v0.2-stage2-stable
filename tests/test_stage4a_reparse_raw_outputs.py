...
def test_reparse_stage4_outputs_recovers_half_compliant_xrd_and_tem(tmp_path: Path) -> None:
    stage4_dir = tmp_path / "stage4_vision_spectra"
    stage4_dir.mkdir(parents=True)

    candidates = [
        {"figure_id": "图2-3", "figure_type": "xrd_pattern", "send_to_vlm": True, "paper_id": "paper", "source_image_path": "xrd.png", "caption": "xrd", "context_source": {}},
        {"figure_id": "图2-5", "figure_type": "tem_image", "send_to_vlm": True, "paper_id": "paper", "source_image_path": "tem.png", "caption": "tem", "context_source": {}},
    ]
    raw_outputs = [
        {
            "figure_id": "图2-3",
            ...
            "raw_response": json.dumps(
                {
                    ...
                    "figure_type": "xrd_pattern",
                    "confidence": "high",
                    "detected_phases": [{"phase": "莫来石", "source": "image_and_text", "confidence": "high"}],
    ...
    write_jsonl([{"figure_id": "图2-3", "error": "old"}, {"figure_id": "图2-5", "error": "old"}], stage4_dir / "failed_records.jsonl")
    ...
    spectra = [json.loads(line) for line in (stage4_dir / "spectra_extractions.jsonl").read_text(encoding="utf-8").splitlines() if line.strip()]
    assert len(spectra) == 2
    xrd = next(item for item in spectra if item["figure_id"] == "图2-3")
    tem = next(item for item in spectra if item["figure_id"] == "图2-5")
    assert xrd["schema_name"] == "XRDExtraction"
    assert xrd["detected_phases"] == ["莫来石"]
