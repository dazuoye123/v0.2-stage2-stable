...
def test_run_stage6c_full_resume_continues_after_paper_failure(monkeypatch, tmp_path: Path) -> None:
    ...
    markdown_dir.mkdir()
    outputs_dir.mkdir()
    for paper_id in ["论文A", "论文B"]:
        (markdown_dir / f"{paper_id}.md").write_text("内容", encoding="utf-8")
        (outputs_dir / paper_id).mkdir()

    candidates = [
        {"paper_id": "论文A", "markdown_path": str(markdown_dir / "论文A.md"), "output_dir": str(outputs_dir / "论文A")},
        {"paper_id": "论文B", "markdown_path": str(markdown_dir / "论文B.md"), "output_dir": str(outputs_dir / "论文B")},
    ]
    statuses = {
        "论文A": _status(stage2=True, stage3=True, stage4a=True, stage5=False, stage55=False),
        "论文B": _status(stage2=True, stage3=False, stage4a=False, stage5=False, stage55=False),
    }
    ...
    def fake_execute(**kwargs):
        if kwargs["paper_id"] == "论文A":
            raise RuntimeError("boom")
        return [{"paper_id": "论文B", "stage": "stage3", "action": "run_stage3", "status": "completed"}]
    ...
    assert any(item["paper_id"] == "论文B" for item in result["per_paper_summary"])
